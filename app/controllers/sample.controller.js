...
exports.findAll = (req, res) => {
    ...
    Sample.find(condition)
        ...
        .catch(err => {
            res.status(500).send({
                message:
                err.message || "Some error occurred whild retrieving samples."
            });
        });
};
...
exports.findOne = (req, res) => {
    ...
    Sample.findById(id)
        .then(data => {
            if(!data)
                res.status(404).send({ message: "Not found sample woth id " + id });
            else res.send(data);
        })
        .catch(err => {
            res
...
exports.update = (req, res) => {
    if (!req.body) {
        return res.status(400).send({
            message: "Data to updatte can not be empty!"
        });
    }

    const id = req.params.id;

    Sample.findByIdAndUpdate(id, req.body, { useFindAndMody: false})
        .then(data => {
            if(!data) {
                res.status(404).send({
...
exports.findAllPublished = (req, res) => {
    Sample.find({ published: true })
      ...
      .catch(err => {
        res.status(500).send({
          message:
            err.message || "Some error occurred while retrieving sample."
        });
      });
};